...
class DatasetSemanticSearch:
    ...
    def preload_model(self) -> None:
        ...
        if self.config.show_progress:
            print(f"✅ Model loaded successfully to {self.config.model_cache_dir}", flush=True)
    ...
    def build_embeddings(self, dataset_registry: List[Dict[str, Any]]) -> None:
        ...
        if self.config.show_progress:
            print("🔄 Building embeddings for dataset registry...", flush=True)
    ...
    def load_embeddings_and_index(self) -> bool:
        ...
        try:
            ...
            if self.config.show_progress:
                print("🔄 Loading precomputed embeddings and index...", flush=True)
            ...
            if self.config.show_progress:
                print(f"✅ Loaded embeddings for {len(self.dataset_metadata)} datasets", flush=True)
            return True
            
        except Exception as e:
            print(f"⚠️ Error loading embeddings: {e}", flush=True)
            return False
    ...
    if not force_rebuild and search_engine.config.cache_embeddings and search_engine.load_embeddings_and_index():
        ...
        if len(search_engine.dataset_metadata) == len(dataset_registry):
            if search_engine.config.show_progress:
                print("✅ Using existing embeddings", flush=True)
            return search_engine
        else:
            if search_engine.config.show_progress:
                print("⚠️ Registry size mismatch, rebuilding embeddings...", flush=True)
